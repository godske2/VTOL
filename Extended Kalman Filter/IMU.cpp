...
///////////QUATERNION ADDED
//virtual float quaternion(float& q1, float& q2, float& q3, float& q4);
float IMUClass::quaternion(float& w, float& x, float& y, float& z){
  uint8_t data[8];
  memset(data, 0, 8);
  x = y = z = w = 0;
  const double scale = (1.0 / (1 << 14));
  /* Read quat data (8 bytes) */
  if (!readRegisters(BNO055_QUATERNION_DATA_W_LSB_ADDR, (uint8_t*)data, sizeof(data))) {
    w = NAN;
    x = NAN;
    y = NAN;
    z = NAN;
